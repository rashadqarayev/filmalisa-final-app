...
function imdbStars(imdb) {
  ...
  for (let i = 1; i <= 5; i++) {
    stars +=
      '<i class="fa-' + (i <= filled ? "solid" : "regular") + ' fa-star"></i>';
  }
  return (
    '<div class="card-imdb">' +
    stars +
    '<span class="card-imdb__score">' +
  ...
  carousel.innerHTML = movies
    .map(function (m, i) {
      return (
        ...
        '">' +
        '<img src="' +
        (m.cover_url || "../../assets/images/home.carusel.jpg") +
        '" alt="' +
        m.title +
        '" />' +
        '<div class="hero-content">' +
        '<span class="hero-genre">' +
        (m.category && m.category.name ? m.category.name : "") +
        "</span>" +
...
function setupCardSlider(wrapperId, interval = 1000) {
  ...
  function updateButtons() {
    ...
    if (nextBtn) {
      ...
      nextBtn.style.pointerEvents =
        scrollLeft < maxScroll - 4 ? "auto" : "none";
    }
    scrollRow.classList.toggle("has-overflow-left", scrollLeft > 4);
    scrollRow.classList.toggle(
      "has-overflow-right",
...
async function init() {
  ...
  try {
    ...
    const heroMovies = categories
      .filter(function (c) {
        return !c.name || c.name.toLowerCase() !== "thriller";
      })
      .flatMap(function (c) {
        return c.movies || [];
      });
    ...
    main.querySelectorAll("section:not(#section1)").forEach(function (s) {
      s.remove();
    });
    main.querySelectorAll(".categories-container").forEach(function (c) {
      c.remove();
    });
